...
def fetch_data_from_senhas_db_financas(query, params=None):
    with connections['senhas_facil_a'].cursor() as cursor:
        cursor.execute(query, params)
        result = cursor.fetchall()
    return result
...
def get_posicao_na_fila_financas(tipo_atendimento_prefixo, numero_senha):
    hoje = datetime.now().date()
    ...
    """
    params = [tipo_atendimento_prefixo, numero_senha]
    result_status = fetch_data_from_senhas_db_financas(query_status, params)
    if not result_status or result_status[0][1] != hoje:
        return 0, 'não encontrado'
    ...
    """
    params_posicao = [tipo_atendimento_prefixo, numero_senha, hoje]
    result_posicao = fetch_data_from_senhas_db_financas(query_posicao, params_posicao)
...
def posicao_na_fila_financas(request, senha):
    resultado = None
    context = {}
    try:        
        tipo_atendimento_prefixo = senha[0]  
        numero_senha = senha[1:]
        posicao, status = get_posicao_na_fila_financas(tipo_atendimento_prefixo, numero_senha)
        resultado = {
            'senha': senha,
            'posicao_na_fila': posicao,
            'status': status
    ...
    context['resultado'] = resultado
    return render(request, 'senha_facil/celular_senhas_chamadas_financas.html', context)
...
def fetch_posicao_na_fila_financas(request, senha):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            print("Recebido do cliente:", data)
